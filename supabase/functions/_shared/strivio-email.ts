...
export function renderStrivioEmail(ctx: StrivioEmailContext): RenderedStrivioEmail {
  ...
  const messageBlock = adminNote
    ? `<div style="margin:10px 0 22px;padding:17px 18px;background:${BRAND.panelSoft};border:1px solid #294524;border-${formattedAdminNote.dir === "rtl" ? "right" : "left"}:3px solid ${BRAND.neon};border-radius:14px"><div style="margin-bottom:9px;color:${BRAND.neon};font-family:Tahoma,'Segoe UI',Arial,sans-serif;font-size:12px;font-weight:900;letter-spacing:.2px">${esc(deliveryNoteTitle)}</div><div dir="${formattedAdminNote.dir}" style="color:${BRAND.text};direction:${formattedAdminNote.dir};unicode-bidi:plaintext;text-align:${formattedAdminNote.dir === "rtl" ? "right" : "left"};font-family:Tahoma,'Segoe UI',Arial,sans-serif;font-size:15px;font-weight:600;white-space:pre-wrap;overflow-wrap:anywhere;line-height:2">${formattedAdminNote.html}</div></div>`
    : "";
